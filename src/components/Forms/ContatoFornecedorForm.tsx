...
interface ContatoFornecedorFormData {
  nome: string;
  email?: string;
  telefone?: string;
  whatsapp?: string;
...
interface ContatoFornecedorFormProps {
  ...
  onClose: () => void;
  onSuccess?: () => void;
  contatoToEdit?: ContatoFornecedor | null; // Contato para edição
}

const ContatoFornecedorForm: React.FC<ContatoFornecedorFormProps> = ({ 
  isOpen,
  fornecedorId, 
  fornecedorNome, 
  onClose, 
  ...
  const { register, handleSubmit, formState: { errors }, setValue, setError, clearErrors, watch } = useForm<ContatoFornecedorFormData>({
    defaultValues: {
      ...
      email: contatoToEdit?.email || '',
			...
			ativo: (contatoToEdit as any)?.ativo || true
    }
  });
  ...
  const onSubmit = async (data: ContatoFornecedorFormData) => {
    ...
    try {
      ...
      if (contatoToEdit) {
        ...
        await contatoFornecedorService.updateContato(fornecedorId, contatoToEdit.id, contatoData);
      } else {
        // Criar novo contato
        await contatoFornecedorService.createContato(fornecedorId, contatoData);
      }
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-500 mt-1">Fornecedor: {fornecedorNome}</p>
          </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-gray-900">Status do Contato</h3>
                    <p className="text-xs text-gray-500">Define se o contato está ativo no sistema</p>
                  </div>
                </div>
                <div className="flex items-center">
                  <button
